...
<?php 
	...
	// $myfirstname = "Jitesh";
	// $mylastname = "Sekar" ;
	// $myemailaddress = "jitshakes@gmail.com";
	// $myzipcode = "560079";
	// $myssn = "456";
	// $myloanamt = "5000";
	// $mycurrentMinPayment = "200";
	// $mycurrentAPR = "678";
	// $myprefminpayment = "456";
	// $myprefloanduration = "5";
	// $mycreditscore = "256";
	// $myLLCompBand = 0;
...
if ($mycreditscore =="5")	
{
	...
	echo json_encode(array(
		'status1' => 0,
		'message' => "Sorry, you do not qualify for a LendLift loan"
		));
}
...
elseif ($mycreditscore == "0") 
{
	...
	echo json_encode(array(
		'status1' => 0,
		'message' => "Sorry, we need to ask you some more questions before we could consider you for a LendLift loan"
		));
...
else
{
	...
	// Simulate the look up from the credit score from bureau.
	$myadjustcreditscore = rand(550,800);
	...
	if (($myadjustcreditscore < 600) || ($inquiries_sixmonths > 4) || ($months_since_delinq > 3) || ($revolving_line_util > 40))
	{
		if( $myadjustcreditscore < 600) {
			echo json_encode(array(
			'status1' => 0,
			'message' => "{$myfirstname}{$mylastname},your up-to-date credit score : {$myadjustcreditscore}<br/>This did not meet our minimum credit requirements for qualifying for a LendLift Loan.
			Please contact us at contact@lendlift.com if you have more questions."
			));
		}else if($inquiries_sixmonths > 4) {
			echo json_encode(array(
			'status1' => 0,
			'message' => "{$myfirstname}{$mylastname},number of credit enquiries in last 6 months : {$inquiries_sixmonths}<br/>This did not meet our minimum credit requirements for qualifying for a LendLift Loan.
			Please contact us at contact@lendlift.com if you have more questions."
			));

		}else if($months_since_delinq > 3) {
			echo json_encode(array(
			'status1' => 0,
			'message' => "{$myfirstname}{$mylastname},number of months since last delinquency : {$months_since_delinq}<br/>This did not meet our minimum credit requirements for qualifying for a LendLift Loan.
			Please contact us at contact@lendlift.com if you have more questions."
			));

		}else if($revolving_line_util > 40) {
			echo json_encode(array(
			'status1' => 0,
			'message' => "{$myfirstname}{$mylastname},your revolving line utilization is : {$revolving_line_util}%<br/>This did not meet our minimum credit requirements for qualifying for a LendLift Loan.
			Please contact us at contact@lendlift.com if you have more questions."
			));	
		}
	...
	else
	{
		...
		$mycalculatedband = 10-abs(round(($mycalculate_compgradescore/350), 0) - 2);
			...
			if ($db_handle)
			{
				...
				if ($query_inputresult)
				{
					...
					while($row = mysql_fetch_array($query_inputresult))
					{
						...
						// print json_encode("LoanAmount = " .$row['Loan Amount']);
						// print json_encode("Term = " .$row['Term']);
						// print json_encode("Origination Fees = " .$row['Orig Fee']);
						// print json_encode("Servicing Fees = " .$row['Svc Fee']);
						// print json_encode("New Rate = " .$row['New Rate']);
						// print json_encode("Effective Rate = " .$row['Effective Rate']);
						// print json_encode("Payment = " .$row['Payment']);
						// print json_encode("Reward Redemption Rate = " .$row['Reward Rdmpt']);
						...
						echo json_encode(array(
						    ...
						    'sFees' =>$row['Svc Fee'],
						    'nRate' => $row['New Rate'],
						    'eRate' => $row['Effective Rate'],
						    'payment' => $row['Payment'],
						    'Reward' =>$row['TPLendlift'],
						    'bRate' => $row['BaseRate'],
						    'adjR' => $row['AdjforRisk'],
						    'uFees' => $row['UnsuccessfulFee'],
						    'lBand' =>$row['Band'],
						    'lFees' => $row['LateFee'],
						    'cpf' =>$row['CheckProcessingFee'],
						    'col60'=>$row['Collection60'],
						    'pFees' => $row['ProcessingFee'],
							// Investor related fields;
							/*'rwd' => $row['RewardRdmpt'],
							'roi' => $row['ROI'],
							'yld' => $row['Ann. Yield'],
							'netyldrwd' => $row['Net Ann after Rewards'],
							'VarImp' => $row['VaRImp']
							*/
