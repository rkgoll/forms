...
else
{
	...
	if (($myadjustcreditscore < 600) || ($inquiries_sixmonths > 4) || ($months_since_delinq > 3) || ($revolving_line_util > 40))
	{
		if( $myadjustcreditscore < 600) {
			...
			echo json_encode(array(
			'status1' => 0,
			'message' => "{$myfirstname}{$mylastname},number of months since last delinquincy : {$months_since_delinq}<br/>This did not meet our minimum credit requirements for qualifying for a LendLift Loan.
			Please contact us at contact@lendlift.com if you have more questions."
			));
